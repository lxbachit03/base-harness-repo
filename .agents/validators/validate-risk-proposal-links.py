#!/usr/bin/env python3
"""Static risk/proposal relationship and risk-only INDEX validator."""
...
class Validator:
    ...
    def snapshot(self) -> Tuple[Tuple[str, str], ...]:
        ...
        for path in sorted(paths, key=lambda item: self.rel(item)):
    ...
    def parse_resource(self, path: Path) -> Resource:
        ...
        for number, line in enumerate(resource.lines, start=1):
            if line.startswith("## "):
                break
    ...
    def validate_metadata(self, resource: Resource) -> None:
        ...
        if not filename_match:
            ...
            if filename_match.group(1) != resource.resource_id[-4:]:
                self.error("FILENAME", path, 0, "filename MMDD must match the immutable ID date")
        ...
        for section in REQUIRED_SECTIONS["risk" if resource.kind == "risks" else "proposal"]:
            if section not in {line.strip() for line in resource.lines if line.startswith("## ") }:
    ...
    def parse_reference_links(self, resource: Resource) -> List[Link]:
        ...
        for index in range(start + 1, end):
            ...
            if not line.startswith("- ") or len(matches) != 1 or line.strip() != "- " + matches[0].group(0):
                self.error("FORMAT", resource.path, index + 1, "REFERENCES must contain only canonical Markdown link bullets")
                continue
    ...
    def parse_section_links(self, resource: Resource, heading: str) -> List[Link]:
        ...
        for index in range(start + 1, end):
            ...
            matches = list(LINK_RE.finditer(line))
            if not line.startswith("- ") or len(matches) != 1 or line.strip() != "- " + matches[0].group(0):
                self.error("FORMAT", resource.path, index + 1, f"{heading} must contain only canonical link bullets")
                continue
            links.append(Link(matches[0].group(1), matches[0].group(2), index + 1))
        if not links:
    ...
    def validate_relationships(self) -> None:
        for resource in self.resources.values():
            ...
            if set(ref_ids) != set(related_ids):
                self.error("RECIPROCAL", resource.path, 0, "REFERENCES and related-resource IDs must be identical")
    ...
    def validate_index_link(self, label: str, target: str, line: int, expected_kind: str, priority: Optional[str] = None) -> Optional[Resource]:
    ...
    def validate_index(self) -> None:
        ...
        resource_risks = {resource.rel: resource for resource in self.resources.values() if resource.kind == "risks"}
        resource_proposals = {resource.rel: resource for resource in self.resources.values() if resource.kind == "proposals"}
        risk_heading = next((i for i, line in enumerate(self.index_lines) if line.strip() == "## TAG: [RISK]"), None)
        ...
        if risk_heading is not None:
            ...
            resources_heading = next(
                (i for i in range(risk_heading + 1, section_end) if self.index_lines[i].strip() == "Resources:"),
                None,
            )
            ...
            if resources_heading is not None:
                for index in range(resources_heading + 1, section_end):
                    ...
                    if top_match:
                        resource = self.validate_index_link(top_match.group(1), top_match.group(2), index + 1, "risks", top_match.group(3))
                        if resource:
                        ...
                        if current is None:
                            ...
                            proposal = self.validate_index_link(nested_match.group(1), nested_match.group(2), index + 1, "proposals")
                            allowed_index_links.add((index + 1, nested_match.group(2)))
                            if proposal:
                                current[2].append(proposal)
                    elif LINK_RE.search(line) and any(part in line for part in ("risks/", "proposals/")):
                            self.error("INDEX", index_path, index + 1, "canonical INDEX links must use the strict risk/nested-proposal grammar")
            else:
        ...
        if [resource.resource_id for _, resource, _ in risk_entries] != sorted(
        ...
        for _, resource, proposals in risk_entries:
            ...
            expected_ids = {link.target_id for link in resource.related if link.target_id}
            ...
            for match in LINK_RE.finditer(line):
                ...
                if relative in resource_risks or relative in resource_proposals:
                    if (index, target) not in allowed_index_links:
                        self.error("INDEX", index_path, index, "canonical resources may only be reached through the risk-only INDEX grammar")

    def run(self) -> int:
        before = self.snapshot()
        self.validate_scope()
        for path in sorted(self.resource_paths, key=lambda item: self.rel(item)):
        ...
        if self.errors:
            for fatal, rule, path, line, message in sorted(self.errors, key=lambda item: (item[2], item[3], item[1], item[4])):
                level = "FATAL" if fatal else "ERROR"
                location = f"{path}:{line}" if line else path
                print(f"{level}|{rule}|{location}|{message}")
            print(f"SUMMARY|status=FAIL|risks={sum(r.kind == 'risks' for r in self.resources.values())}|proposals={sum(r.kind == 'proposals' for r in self.resources.values())}|exit={code}")
        else:
